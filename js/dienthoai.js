...
const Xuat_San_pham_Dienthoai = (ds = [], tag, nhom) => {
    let html = ``
    ds.forEach(item => {
        html += `
        <div class="col mb-5">
...
const Xuat_Tivi_theo_Nhom = (maNhom) => {
    // dsLoc=dsDienthoai;
    if (maNhom != "ALL") {
        dsLoc = dsDienthoai.filter(x => x.Nhom.Ma_so == maNhom);
    ...
    if (key == "Ten") {
        ...
        if (sort == "1") {
            ...
            if (danhsach.length > 8) {
                ...
                Xuat_San_pham_Dienthoai(dsLoc, Th_San_pham_Dienthoai)
            }
        } else {
            tag.innerHTML = `Tên &blacktriangle;`
            tag.setAttribute("sort", "1");
            danhsach = dsLoc.sort((a, b) => {
...
const Loc_Gia = () => {
    ...
    //     Xuat_San_pham_Dienthoai(ds,Th_San_pham_Dienthoai)
    // }
    ...
    locGia.addEventListener("input", (e) => {
        ...
        let ds = dsLoc.filter(x => Number(x.Don_gia_Ban) <= Number(e.target.value))
...
tim.onclick = () => {
    if (timKiem.value != "") {
        dsTim = dsLoc.filter(x => x.Ten.toLowerCase().includes(timKiem.value.toLowerCase()));
        if (dsTim.length > 8) {
            dsLoc = dsTim;
            xuat_nut_phan_trang(dsLoc, show_paginate);
            document.getElementById("1").click();
        } else {
            Xuat_San_pham_Dienthoai(dsTim, Th_San_pham_Dienthoai)
        }    
    } else {
        dsLoc = dsDienthoai;
        xuat_nut_phan_trang(dsLoc, show_paginate);
...
timKiem.onkeyup = (e) => {
    if (e.keyCode == 13 && e.target.value != "") {
        dsTim = dsLoc.filter(x => x.Ten.toLowerCase().includes(e.target.value.toLowerCase()));
        if (dsTim.length > 8) {
            dsLoc = dsTim;
            xuat_nut_phan_trang(dsLoc, show_paginate);
            document.getElementById("1").click();
...
const pagination = (number, nhom) => {
    ...
    Xuat_San_pham_Dienthoai(currentPosts, Th_San_pham_Dienthoai, nhom);
}
